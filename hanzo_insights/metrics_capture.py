...
cross-window state. Mirrors the ``insights-js`` core implementation so every
...
from hanzo_insights.request import _get_session
...
# latency/size ranges without per-metric configuration. Must match insights-js.
...
class InsightsMetrics:
    ...
    ``flush_interval`` — unlike insights-js, whose ``flushIntervalMs`` is milliseconds)::

        client = Client("<project_api_key>", metrics={"service_name": "billing-worker"})
    ...
    def _reset_after_fork_locked(self) -> None:
        # A forked child inherits the parent's window and a timer handle whose thread
        # does not exist in the child — without this, the child never flushes (silent
        # total loss) and would duplicate the parent's samples if it ever did. Drop both.
        pid = os.getpid()
        if pid == self._pid:
            return
        self._pid = pid
